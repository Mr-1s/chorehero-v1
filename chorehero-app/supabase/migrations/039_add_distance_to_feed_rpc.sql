-- Migration 039: Add distance_km to get_ranked_cleaner_feed for distance pill display
-- Must DROP first: PostgreSQL does not allow changing return type with CREATE OR REPLACE

DROP FUNCTION IF EXISTS get_ranked_cleaner_feed(double precision, double precision, double precision, integer, boolean);

CREATE FUNCTION get_ranked_cleaner_feed(
...
BEGIN
  ...
  SELECT
    ...
    CASE
      WHEN p_lat IS NOT NULL AND p_lng IS NOT NULL AND ca.latitude IS NOT NULL AND ca.longitude IS NOT NULL
      THEN SQRT(POWER((ca.longitude::float - p_lng) * 111.0 * COS(RADIANS(p_lat)), 2) +
           POWER((ca.latitude::float - p_lat) * 111.0, 2))::float
      ELSE NULL::float
    END AS distance_km
