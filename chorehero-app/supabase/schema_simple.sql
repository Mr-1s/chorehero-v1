-- ChoreHero Database Schema (Simplified - No PostGIS)
...
-- Addresses table (simplified without PostGIS)
...
-- Cleaner availability table (simplified time format)
CREATE TABLE public.cleaner_availability (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  cleaner_id UUID REFERENCES public.users(id) ON DELETE CASCADE,
  day_of_week INTEGER CHECK (day_of_week >= 0 AND day_of_week <= 6), -- 0 = Sunday
  start_time VARCHAR(8) NOT NULL, -- Format: "09:00:00"
  end_time VARCHAR(8) NOT NULL,   -- Format: "17:00:00"
  is_available BOOLEAN DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- Location tracking table (simplified without PostGIS)
