...
const CustomerOnboardingScreen: React.FC<CustomerOnboardingProps> = ({ navigation }) => {
  ...
  const handleUsernameChange = (text: string) => {
    updateData('username', text.toLowerCase().replace(/[^a-z0-9_]/g, ''));
  ...
  const renderStep1 = () => (
    ...
    >
      ...
      <View style={styles.emailContainer}>
        <TextInput
          style={[styles.textInput, data.email && authUser ? styles.readOnlyInput : null]}
          value={data.email}
          onChangeText={(text) => updateData('email', text)}
          placeholder="john.doe@example.com"
          keyboardType="email-address"
          autoCapitalize="none"
          editable={!authUser?.user?.email}
        />
        {authUser?.user?.email && (
          <Ionicons name="checkmark-circle" size={20} color="#00D4AA" style={styles.verifiedIcon} />
        )}
      </View>
      ...
      <TextInput
        ...
        onFocus={() => setTimeout(() => scrollRef.current?.scrollTo({ y: 250, animated: true }), 100)}
      />
      ...
      <View style={styles.usernameContainer}>
        <TextInput
          ...
          autoCapitalize="none"
          maxLength={20}
          onFocus={() => setTimeout(() => scrollRef.current?.scrollTo({ y: 350, animated: true }), 100)}
        />
        <View style={styles.usernameStatusContainer}>
          {usernameStatus === 'checking' && (
            <Text style={styles.usernameChecking}>Checking...</Text>
          )}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Current Step Content - outside KeyboardAvoidingView for better scroll */}
      <View style={styles.content}>
        {renderCurrentStep()}
      </View>

      {/* Bottom Button - wrapped in KeyboardAvoidingView to stay above keyboard */}
      <KeyboardAvoidingView 
...
const styles = StyleSheet.create({
  ...
  keyboardAvoid: {
    // Don't take up extra space, just handle positioning
  },
  stepContainer: {
    flex: 1,
    paddingHorizontal: 20,
